...
- UI 整理：用 UMG 工具面板替代旧 Slate 拼接界面，统一工具窗口布局、视觉层级与 RT 预览区域。
- 工具交互逻辑优化：梳理导入、动作加载、物理烘焙、预览播放、状态反馈等操作入口，使按钮行为和工具流程更清晰。
- 暂缓底层算法扩展：当前阶段优先保证工具窗口可用性、交互闭环和预览体验，物理/动画算法只做必要接入与验证。
